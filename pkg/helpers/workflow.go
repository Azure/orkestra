...
func (helper *ReconcileHelper) cleanupWorkflow(ctx context.Context, logr logr.Logger, wf *v1alpha12.Workflow) error {
	nodes := make(map[string]v1alpha12.NodeStatus)

	// suspend the forward workflow if it is still running
	err := helper.suspendWorkflow(ctx, logr, wf)
	if err != nil {
	...
	if err != nil {
		if kerrors.IsNotFound(err) {
			...
			engine, _ := helper.EngineBuilder.Reverse(wf, nodes).Build()
